...
/* 05 · CURRENT EXPLORATION — threads, not products. */
export function Exploration() {
  return (
    <section id="exploration" aria-labelledby="exploration-title" className="content-auto relative px-6 py-28 lg:px-12 lg:py-40">
      <div className="mx-auto max-w-6xl">
        <SectionHeader
          index="05"
          label="Current exploration"
          title={<span id="exploration-title">Open threads.</span>}
        ...
        <ol className="mt-16 border-t border-line-soft">
          {explorationThemes.map((t, i) => (
            <Reveal key={t.id} delay={i * 0.04}>
              <li className="grid gap-4 border-b border-line-soft py-8 lg:grid-cols-12 lg:gap-8 lg:py-10">
                <div className="flex items-start gap-5 lg:col-span-4">
                  <span className="mono mt-1 text-[11px] tracking-[0.2em] text-ghost">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <div>
                    <h3 className="display text-2xl text-ink">{t.title}</h3>
                    <div className="mt-3">
                ...
                <ul className="flex flex-wrap gap-2 lg:col-span-3 lg:justify-end lg:self-start">
                  {t.threads.map((th) => (
                    <li key={th} className="mono border border-line px-2.5 py-1.5 text-[11px] text-faint">
                      {th}
                    </li>
                  ))}
                </ul>
